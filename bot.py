...
# ═══════════════════════════════════════════════════════════════════════════════
# MULTI-BOT KATMANI — CEO orkestrasyon + kanal topic desteği
# ═══════════════════════════════════════════════════════════════════════════════
...
def send_to_topic(topic_id: int, text: str, parse_mode: str = 'Markdown') -> bool:
    """Kanalın belirtilen topic'ine CEO bot üzerinden mesaj gönder."""
    try:
        bot.send_message(
            CHANNEL_ID,
            text,
...
def _register_sub_webhooks(render_url: str):
    """Alt-botların Flask webhook route'larını kaydet."""
    sub_bots = [
    ...
    for name, bot_obj, token in sub_bots:
        if not bot_obj or not token:
            continue
        def make_view(b=bot_obj, n=name):
            def view():
                if request.headers.get('content-type') == 'application/json':
                    update = telebot.types.Update.de_json(request.get_data(as_text=True))
                    threading.Thread(target=b.process_new_updates, args=([update],), daemon=True).start()
                    return 'OK', 200
                abort(403)
            view.__name__ = f'webhook_{n}'
            return view
        app.add_url_rule(f'/webhook/{token}', view_func=make_view(), methods=['POST'])
        print(f'[WEBHOOK] {name} route kaydedildi.')

def _set_sub_webhooks(render_url: str):
    """Alt-botların Telegram webhook'larını ayarla."""
    sub_bots = [
        ('scanner',   _scanner_bot,   SCANNER_BOT_TOKEN),
        ('news',      _news_bot,      NEWS_BOT_TOKEN),
        ('tradebook', _tradebook_bot, TRADEBOOK_BOT_TOKEN),
    ...
    for name, bot_obj, token in sub_bots:
        ...
        try:
            ...
            time.sleep(0.5)
            bot_obj.set_webhook(url=f'{render_url}/webhook/{token}')
...
GEMINI_PRO_KEY = (os.getenv('GEMINI_PRO_KEY') or '').strip()  # ProfGenBot — Gemini 2.5 Pro
...
app = Flask(__name__)
_register_sub_webhooks(RENDER_URL)  # Alt-bot route'ları (ön-kayıt)
...
def set_webhook():
    ...
    _set_sub_webhooks(RENDER_URL)  # Alt-bot webhook'ları
    _set_bot_commands()
...
GEMINI_PRO_MODELS = ["gemini-2.5-pro-preview-05-06","gemini-2.5-pro","gemini-2.0-pro"]  # ProfGenBot
...
def _gemini_keys_pro():
    """ProfGenBot için Gemini Pro anahtarı. Yoksa normal key havuzuna düşer."""
...
def gemini_ask_pro(prompt, max_tokens=2000):
    """ProfGenBot için Gemini Pro modeli ile istek gönder."""
    keys = _gemini_keys_pro()
    if not keys: return gemini_ask(prompt, max_tokens)
    payload = {"contents":[{"parts":[{"text":prompt}]}],
               "generationConfig":{"maxOutputTokens":max_tokens,"temperature":0.3}}
    for key_no, api_key in keys:
        if key_no != 0 and _gemini_key_exhausted(key_no): continue
        for model in GEMINI_PRO_MODELS:
            url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={api_key}"
            try:
                resp = requests.post(url, json=payload, timeout=45)
                if resp.status_code == 429:
                    if key_no != 0: _gemini_mark_exhausted(key_no)
                    continue
                if resp.status_code in (400, 401, 403): continue
                resp.raise_for_status()
                data = resp.json()
                return data["candidates"][0]["content"]["parts"][0]["text"].strip()
            except Exception: continue
    return gemini_ask(prompt, max_tokens)  # Fallback: normal key havuzu
...
# ═══════════════════════════════════════════════════════════════════════════════
# UYGULAMA BAŞLANGIÇ — gunicorn "bot:app" yaptığında bu blok çalışır
# ═══════════════════════════════════════════════════════════════════════════════
def _app_startup():
    """Gunicorn worker başladığında DB, webhook ve thread'leri başlatır."""
    import sys
...
# gunicorn bot:app → modül import edilince bu satır çalışır
_app_startup()


def main():
    """Direkt python bot.py ile çalıştırıldığında (polling modu)."""
    _app_startup()
    if not RENDER_URL:
